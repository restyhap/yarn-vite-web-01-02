...
const processImage = async (imageUrl: string, workbook: ExcelJS.Workbook, colStart: number, colEnd: number, imageRow: number) => {
  try {
    ...
    // 修改图片添加方式，使用固定单元格和固定尺寸
    return {
      tl: { col: colStart, row: imageRow },
      br: { col: colEnd, row: imageRow + 1 },
      editAs: 'oneCell'  // 添加这个属性使图片固定在单元格中
    }
  } catch (error) {
...
export const exportQuotation = async (formData: QuotationData) => {
  ...
  // 添加 logo 到标题行右上角
  const logoBuffer = base64ToUint8Array(LOGO)
      ...
      if (formData.image) {
        try {
          ...
          if (formData.image.startsWith('data:image')) {
            ...
            // 使用代理获取图片数据
            imageBuffer = await getImageBuffer(formData.image);
          }
          ...
          if (formData.image.startsWith('data:image')) {
            ...
            imageExtension = (mimeType === 'jpeg' || mimeType === 'jpg') ? 'jpeg' :
                           mimeType === 'png' ? 'png' :
                           mimeType === 'gif' ? 'gif' : 'jpeg';
          }
          ...
          // 将图片添加到工作表
          worksheet.addImage(imageId, 'B13:B13');
        } catch (error) {
