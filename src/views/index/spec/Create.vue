        ...
        >
          ...
          <div class="mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 lg:gap-3">
              ...
              <el-form-item 
                label="评价内容" 
                class="col-span-3 bg-gray-100 p-1 rounded flex flex-col sm:flex-row items-start sm:items-center !mb-2" 
                label-position="left"
              >
                <el-input
          ...
          <div class="mb-8" v-if="formData.defectsDTO.length > 0">
            ...
            <!-- 缺陷记录列表 -->
            <div class="space-y-4">
              <div v-for="(defect, index) in formData.defectsDTO" 
                   :key="defect.defects.id" 
              ...
              >
                  ...
                  <div class="flex gap-2">
                    <template v-if="defect.isEditing">
                      <el-button 
                        type="success"
                        size="small"
                        class="bg-green-500 hover:bg-green-600 border-0 shadow-sm !h-8"
                        @click="saveDefectEdit(index)"
                      >
                      ...
                      <el-button 
                        size="small"
                        class="bg-gray-200 hover:bg-gray-300 border-0 shadow-sm !h-8"
                        @click="cancelDefectEdit(index)"
                      >
                        取消
                      </el-button>
                    </template>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  >
                    <template v-if="defect.isEditing">
                      <el-input 
                        v-model="defect.editData.defects.defectTitle" 
                        placeholder="请输入缺陷标题"
                        class="w-full !h-[38px]"
                      />
                    </template>
                  ...
                  >
                    <template v-if="defect.isEditing">
                      <el-input 
                        v-model="defect.editData.defects.inspector" 
                        placeholder="请输入检查人员"
                        class="w-full !h-[38px]"
                      />
                    </template>
                  ...
                  <el-form-item 
                    label="缺陷描述" 
                    class="col-span-2 bg-gray-100 p-1 rounded flex flex-col sm:flex-row items-start sm:items-center !mb-2" 
                    label-position="left"
                  >
                    <template v-if="defect.isEditing">
                      <el-input
                        v-model="defect.editData.defects.defectDescription"
                        placeholder="请输入缺陷描述"
                        class="w-full !h-[38px]"
                      />
                    </template>
                  ...
                  >
                    <template v-if="defect.isEditing">
                      <el-input
                        v-model="defect.editData.defects.improvementSuggestion"
                        placeholder="请输入改进建议"
                        class="w-full !h-[38px]"
                      />
                    </template>
                  ...
                  <el-form-item 
                    label="缺陷图片" 
                    class="col-span-2 bg-gray-100 p-2 rounded flex flex-col sm:flex-row items-start sm:items-center !mb-2" 
                    label-position="left"
                  >
                    <div class="w-full">
                      <template v-if="defect.isEditing">
                        <ImageHandler
                          v-model="defect.editData.defectImages"
                          :limit="2"
                          :size="120"
                          :editable="true"
                          @preview="handlePreview"
                      ...
                      <template v-else>
                        <ImageHandler
                          :model-value="defect.defectImages.map(img => img.imagePath)"
                          :size="120"
                          :editable="false"
                          @preview="handlePreview"
                        />
          ...
          >
            <el-form :model="currentDefect" label-width="100px">
              <div class="grid grid-cols-1 gap-4">
                ...
                >
                  <el-input 
                    v-model="currentDefect.defects.defectTitle" 
                    placeholder="请输入缺陷标题"
                    class="w-full !h-[38px]"
                  />
                </el-form-item>
                ...
                >
                  <el-input 
                    v-model="currentDefect.defects.inspector" 
                    placeholder="请输入检查人员"
                    class="w-full !h-[38px]"
                  />
                </el-form-item>
            ...
            <template #footer>
              <div class="flex justify-end gap-4">
                ...
                <el-button 
                  ...
                  @click="defectDialogVisible = false"
                >
                  取消
                </el-button>
              </div>
...
const saveDefect = () => {
  ...
  // 创建新的缺陷记录对象
  const newDefect: DefectsDTO & { isEditing?: boolean; editData?: DefectEditData } = {
    defects: {
...
const fillTestData = () => {
  ...
  formData.value.qcReports = {
    ...
    secondQcDate: new Date(Date.now() + 24 * 60 * 60 * 1000), // 明天
    comments: [
      '产品整体质量良好，符合标准要求',
      '存在少量瑕疵，但在可接受范围内',
      '部分细节需要改进，建议加强质量控制',
