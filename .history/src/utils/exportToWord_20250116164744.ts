      ...
      children: [
        new TableCell({
          ...
          children: [
            ...
            new Paragraph({
              text: '(MINIMUM TWIN WALLED CARDBOARD',
              alignment: AlignmentType.CENTER
            }),
            new Paragraph({
              text: 'WITH DIVIDERS)',
 ...
 * @param label - 标签文本
 * @param value - 值文本
 * @param subLabel - 子标签文本（用于 LOADING QUANTITY 和 WEIGHT 的子项）
 * @param isHeader - 是否是标题行
 * @returns TableRow - 返回一个表格行对象
 */
const createProductTableRow = (label: string, value: string | undefined, subLabel?: string, isHeader = false) => {
  if (isHeader) {
    return new TableRow({
      children: [
        new TableCell({
    ...
    // 对于有子标签的行（如 LOADING QUANTITY 和 WEIGHT）
    return new TableRow({
      children: [
        new TableCell({
          width: {
      ...
      sections: [{
        ...
        children: [
          ...
          createTable([
            ...
            createProductTableRow('LOADING QUANTITY', '', '20\'GP', false),
            createProductTableRow('', '180', '40\'HC'),
            createProductTableRow('WEIGHT (KG)', '', 'NET'),
            createProductTableRow('', '10.8KG', 'GROSS'),
            createProductTableRow('', '12.4KG'),
