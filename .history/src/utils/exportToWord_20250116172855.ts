    ...
    const doc = new Document({
      sections: [{
        ...
        children: [
          ...
          // 基本信息表
          createTable([
            createTableRow('TCCODE', formData.basic?.tccode),
            createTableRow('SUPPLIER', formData.basic?.supplier),
            createTableRow('SUPPLIER CODE', formData.basic?.supplier_code),
            createTableRow('SUPPLIER NAME', formData.basic?.supplier_name),
            createTableRow('FIRE STANDARD', formData.basic?.fire_standard),
            createTableRow("FOB 20' CONTAINER PRICE", formData.basic?.fob_20_container_price),
            createTableRow("FOB 40'HC CONTAINER PRICE", formData.basic?.fob_40_container_price),
            createTableRow('SHIPPING PORT', formData.basic?.shipping_port)
          ], 80),
          // 单位说明
          new Paragraph({
            children: [
          ...
          createTable([
            ...
            // CARTON 部分
            createCartonTableRow('WIDTH', (formData.carton?.width ? formData.carton.width + 'MM' : 'N/A'), true),
            createCartonTableRow('DEPTH', (formData.carton?.depth ? formData.carton.depth + 'MM' : 'N/A')),
            createCartonTableRow('HEIGHT', (formData.carton?.height ? formData.carton.height + 'MM' : 'N/A')),
            createCartonTableRow('BOARD TYPE', formData.carton?.board_type || 'N/A'),
            createCartonTableRow('Items per carton', (formData.carton?.items_per_carton ? formData.carton.items_per_carton + 'pc' : 'N/A')),
            createCartonTableRow('CARTON VOLUME (m³)', formData.carton?.volume || 'N/A'),

            // PRODUCT 部分
            createProductTableRow('PRODUCTION TIME (DAYS)', (formData.product?.production_time ? formData.product.production_time + 'Day' : 'N/A'), undefined, true),
            createProductTableRow('EFFECTIVE VOLUME (m³)', formData.product?.effective_volume || 'N/A'),
            createProductTableRow('LOADING QUANTITY', formData.product?.loading_quantity_20gp || 'N/A', '20\'GP', false, true),
            createProductTableRow('', formData.product?.loading_quantity_40hc || 'N/A', '40\'HC'),
            createProductTableRow('WEIGHT (KG)', (formData.product?.weight_net ? formData.product.weight_net + 'KG' : 'N/A'), 'NET', false, true),
            createProductTableRow('', (formData.product?.weight_gross ? formData.product.weight_gross + 'KG' : 'N/A'), 'GROSS'),
            ...
            createProductDimensionsTableRow('FROM SEAT', (formData.dimensions?.seat_height_from_seat ? formData.dimensions.seat_height_from_seat + 'MM' : 'N/A'), 'SEAT HEIGHT', false, { isSecondColumnStart: true, rowSpan: 1, secondColumnText: 'SEAT HEIGHT' }),
            ...
            createProductDimensionsTableRow('HEIGHT', (formData.dimensions?.overall_height ? formData.dimensions.overall_height + 'MM' : 'N/A')),
          ], 100),
        ],
      }],
    })
