import { Document, Packer, Paragraph, Table, TableCell, TableRow, TextRun, AlignmentType, HeadingLevel, BorderStyle, VerticalAlign, WidthType } from 'docx'
 ...
 * 创建三列表格行（带合并功能）
 */
const createThreeColumnTableRow = (first: string, second: string, third: string, hasShading = true, mergeCols?: 'middle' | 'last') => {
  const cells = []
  
  // 第一列
  cells.push(new TableCell({
    width: COLUMN_WIDTHS[mergeCols ? (mergeCols === 'middle' ? 'merge_middle' : 'merge_last') : 'standard'].first,
    children: [new Paragraph({
      children: [new TextRun({ text: first, size: 16, bold: true })],
      spacing: { before: 80, after: 80 }
    })],
    verticalAlign: VerticalAlign.CENTER,
    shading: hasShading ? { fill: 'F0F0F0' } : undefined
  ...
  // 第二列
  cells.push(new TableCell({
    width: COLUMN_WIDTHS[mergeCols ? (mergeCols === 'middle' ? 'merge_middle' : 'merge_last') : 'standard'].second,
    children: [new Paragraph({
      children: [new TextRun({ text: second, size: 16, bold: true })],
      spacing: { before: 80, after: 80 }
    })],
    verticalAlign: VerticalAlign.CENTER
  }))
  ...
  if (!mergeCols) {
    // 标准四列布局
    cells.push(new TableCell({
      width: COLUMN_WIDTHS.standard.third,
      children: [new Paragraph({
        children: [new TextRun({ text: third, size: 16, bold: true })],
        spacing: { before: 80, after: 80 }
      })],
      verticalAlign: VerticalAlign.CENTER
    }))
    cells.push(new TableCell({
      width: COLUMN_WIDTHS.standard.fourth,
    ...
    // 合并布局
    cells.push(new TableCell({
      width: COLUMN_WIDTHS[mergeCols === 'middle' ? 'merge_middle' : 'merge_last'].third,
      children: [new Paragraph({
        children: [new TextRun({ text: third, size: 16, bold: true })],
        spacing: { before: 80, after: 80 }
      })],
      verticalAlign: VerticalAlign.CENTER,
      columnSpan: mergeCols === 'middle' ? 2 : 1
    }))
  }
    ...
    const doc = new Document({
      sections: [{
        ...
        children: [
          ...
          // CASTORS 表格
          createTable([
            createThreeColumnTableRow('CASTORS', 'Description', formData.castors?.description || 'N/A', true, 3),
