...
const getLogoBuffer = async (): Promise<Uint8Array> => {
  try {
    ...
    if (buffer.length === 0) {
      console.error('Logo buffer is empty')
      throw new Error('Logo buffer is empty')
    }
    
    console.log('Logo loaded successfully, buffer size:', buffer.length)
    return buffer
  } catch (error) {
    console.error('Failed to load logo:', error)
    throw error  // 让错误继续传播，这样我们可以知道具体是什么问题
  }
}
