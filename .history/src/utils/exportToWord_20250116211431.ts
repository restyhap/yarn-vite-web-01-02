 ...
 * 创建 FOAM 表格的特殊行
 */
const createFoamTableRow = (formData: any, rowType: 'header' | 'density' | 'thickness'): TableRow[] => {
  const firstColumn = new TableCell({
    rowSpan: 5,
    width: {
      size: 25,
      type: 'pct',
  ...
  const createEmptyCell = (width: number) => new TableCell({
    width: {
      size: width,
      type: 'pct',
    },
