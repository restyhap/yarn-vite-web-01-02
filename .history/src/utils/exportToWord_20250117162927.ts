 ...
 * 创建三列表格行（带合并功能）
 */
const createThreeColumnTableRow = (first: string, second: string, third: string, hasShading = true, rowCount?: number) => {
  const cells = []
  
  // 第一列
  cells.push(new TableCell({
    width: COLUMN_WIDTHS[rowCount && rowCount > 3 ? 'merge_middle' : 'standard'].first,
    children: [new Paragraph({
  ...
  // 第二列
  cells.push(new TableCell({
    width: COLUMN_WIDTHS[rowCount && rowCount > 3 ? 'merge_middle' : 'standard'].second,
    children: [new Paragraph({
  ...
  if (!rowCount || rowCount <= 3) {
    ...
    // 合并布局
    cells.push(new TableCell({
      width: COLUMN_WIDTHS['merge_middle'].third,
      children: [new Paragraph({
        children: [new TextRun({ text: third, size: 16, bold: true })],
        spacing: { before: 80, after: 80 }
      })],
