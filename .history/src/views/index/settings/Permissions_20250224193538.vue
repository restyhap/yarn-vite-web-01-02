              ...
              <div class="grid grid-cols-4 gap-8">
                <div v-for="action in module.actions" :key="action.id">
                  <el-checkbox
                    ...
                    @change="handleActionChange($event, role, module.id, action.id)"
                  >
                    {{ action.name }}
                  </el-checkbox>
                </div>
...
const getModuleChecked = (role: IPermission, moduleId: string): boolean => {
  const key = `${moduleId}View` as keyof IPermission;
  return role[key] === 1;
};
...
const handleActionChange = (checked: boolean, role: IPermission, moduleId: string, actionId: string): void => {
  const key = `${moduleId}${actionId}` as keyof IPermission;
  role[key] = checked ? 1 : 0;

  // 更新模块的全选状态
  const module = permissionModules.value.find(m => m.id === moduleId);
  if (!module) return;

  const viewKey = `${moduleId}View` as keyof IPermission;
  const allChecked = module.actions.every(action => {
    const actionKey = `${moduleId}${action.id}` as keyof IPermission;
    return role[actionKey] === 1;
  });
  
  role[viewKey] = allChecked ? 1 : 0;
};
