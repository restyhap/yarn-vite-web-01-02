    ...
    <div class="bg-white rounded-lg shadow p-6">
      ...
      <div class="grid grid-cols-3 gap-6">
        <div v-for="role in roles" :key="role.id" class="border rounded-lg p-6">
          ...
          <!-- 权限模块 -->
          <div class="space-y-4" :class="{ 'disabled-permissions': !isEditing }">
            <div v-for="module in permissionModules" :key="module.id" class="border-t pt-4">
              <div class="flex items-center mb-2">
                <el-checkbox
                  ...
                  :disabled="!isEditing"
                  @change="(val) => handleModuleChange(val, role, module.id)"
                >
              ...
              <div class="grid grid-cols-4 gap-8">
                <div v-for="action in module.actions" :key="action.id">
                  <el-checkbox
                    ...
                    :disabled="!isEditing"
                    @change="(val) => handleActionChange(val, role, module.id, action.id)"
                  >
                    {{ action.name }}
                  </el-checkbox>
                </div>
...
/* 非编辑状态下的文字颜色 */
.disabled-permissions :deep(.el-checkbox__label) {
  color: #1f2937 !important; /* 使用 Tailwind 的 gray-800 颜色 */
}

/* 非编辑状态下的复选框颜色 */
.disabled-permissions :deep(.el-checkbox__input.is-disabled .el-checkbox__inner) {
  background-color: #e5e7eb !important; /* 使用 Tailwind 的 gray-200 颜色 */
  border-color: #d1d5db !important; /* 使用 Tailwind 的 gray-300 颜色 */
}
</style> 