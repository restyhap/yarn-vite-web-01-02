  ...
  <div class="flex h-full">
    ...
    <div class="flex-1 p-6 overflow-y-auto">
      <div class="bg-white rounded-lg shadow p-6">
        ...
        <!-- 改为横向排列的角色列表 -->
        <div class="grid grid-cols-3 gap-6">  <!-- 修改为 grid-cols-3 -->
          <div v-for="role in roles" :key="role.id" class="border rounded-lg p-6">
            ...
            <div class="space-y-4">  <!-- 调整间距 -->
              <div v-for="module in permissionModules" :key="module.id" class="border-t pt-4">
                ...
                <div class="ml-6 grid grid-cols-2 gap-2">  <!-- 修改为 grid-cols-2 -->
    ...
    <div class="w-80 border-l bg-gray-50 p-4 overflow-y-auto">
      <h3 class="text-lg font-medium mb-4">权限说明</h3>
      <div class="space-y-4">
        <div v-for="module in permissionModules" :key="module.id" class="bg-white p-4 rounded-lg shadow-sm">
          <h4 class="font-medium mb-2 text-gray-800">{{ module.name }}</h4>
...
// 检查模块权限是否部分选中
const isIndeterminate = (roleId: string, moduleId: string) => {
  ...
  const selectedCount = permissions.filter(p => role.permissions[`${moduleId}_${p.id}`]).length;
  
  return selectedCount > 0 && selectedCount < permissions.length;
};

// 处理模块权限变更
const handleModuleChange = (checked: boolean, roleId: string, moduleId: string) => {
  const role = roles.value.find(r => r.id === roleId);
  if (!role) return;
...
// 处理单个权限变更
const handlePermissionChange = (roleId: string, moduleId: string) => {
  const role = roles.value.find(r => r.id === roleId);
  if (!role) return;
  ...
  const allChecked = module.permissions.every(p => role.permissions[`${moduleId}_${p.id}`]);
