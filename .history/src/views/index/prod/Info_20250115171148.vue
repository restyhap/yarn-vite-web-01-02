 ...
 * @LastEditTime: 2025-01-15 15:58:20
...
const handleExportWord = async () => {
  try {
    ...
    const doc = new DocxDocument({
      sections: [{
        ...
        children: [
          // 标题
          new Paragraph({
            text: `产品规格书 - ${productInfo.value.tccode}`,
          ...
          createTable([
            ['产品代码', productInfo.value.tccode],
            ['供应商', productInfo.value.supplier],
            ['供应商代码', productInfo.value.supplier_code],
            ['供应商名称', productInfo.value.supplier_name],
            ['防火标准', productInfo.value.fire_standard],
            ['发货港口', productInfo.value.shipping_port],
            ['20尺柜FOB价格', `¥${productInfo.value.fob_20_container_price}`],
            ['40尺柜FOB价格', `¥${productInfo.value.fob_40_container_price}`]
          ]),
          ...
          createTable([
            ['面料制造商', specTables.value.upholstery?.fabric_manufacturer],
            ['颜色代码', specTables.value.upholstery?.colour_code],
            ['皮革等级', specTables.value.upholstery?.leather_grade],
            ['每把椅子用料', `${specTables.value.upholstery?.usage_per_chair}m²`]
          ]),
          ...
          createTable([
            ['包装箱宽度', `${specTables.value.carton?.width}mm`],
            ['包装箱深度', `${specTables.value.carton?.depth}mm`],
            ['包装箱高度', `${specTables.value.carton?.height}mm`],
            ['纸板类型', specTables.value.carton?.board_type],
            ['每箱数量', `${specTables.value.carton?.items_per_carton}件`],
            ['箱体体积', `${specTables.value.carton?.carton_volume}m³`]
          ]),
          ...
          createTable([
            ['生产时间', `${specTables.value.logistics?.production_time}天`],
            ['有效体积', `${specTables.value.logistics?.effective_volume}m³`],
            ['20尺标准柜装载数量', `${specTables.value.logistics?.loading_quantity_20gp}件`],
            ['40尺高柜装载数量', `${specTables.value.logistics?.loading_quantity_40hc}件`],
            ['净重', `${specTables.value.logistics?.net_weight}kg`],
            ['毛重', `${specTables.value.logistics?.gross_weight}kg`]
          ])
        ]
      }]
    })
    ...
    saveAs(buffer, `产品规格书_${productInfo.value.tccode}_${new Date().toISOString().split('T')[0]}.docx`)
