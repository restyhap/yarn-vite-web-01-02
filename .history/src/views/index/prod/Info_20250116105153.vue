      ...
      <template #default>
        <div class="content-wrapper" v-if="formData">
          ...
          <el-tabs v-model="activeTab" class="info-tabs">
            ...
            <!-- 面料信息标签页 -->
            <el-tab-pane label="面料信息" name="upholstery">
              <div class="tab-content">
                <div class="section">
                  <div class="section-header">
                    <div class="actions" v-if="isEditMode">
                      <template v-if="editingSection === 'upholstery'">
                        <el-button type="success" @click="handleSave('upholstery')">
                          <el-icon><Check /></el-icon>
                          保存
                        </el-button>
                        <el-button type="danger" @click="handleCancel('upholstery')">
                          <el-icon><Close /></el-icon>
                          取消
                        </el-button>
                      </template>
                      <template v-else>
                        <el-button type="primary" @click="handleEdit('upholstery')">
                          <el-icon><Edit /></el-icon>
                          编辑
                        </el-button>
                      </template>
                  ...
                  <el-form :model="formData.upholstery" label-width="120px">
                    <el-row :gutter="20">
                      <el-col :span="8" v-for="(field, key) in specSections.upholstery" :key="key">
                        <el-form-item :label="field">
                          <template v-if="editingSection === 'upholstery'">
                            <el-input v-model="formData.upholstery[key]" />
                          </template>
                          <template v-else>
                            {{ formData.upholstery?.[key] || '-' }}
                          </template>
                        </el-form-item>
                      </el-col>
                    </el-row>
            ...
            <!-- 其他标签页类似... -->
            <el-tab-pane v-for="(section, sectionKey) in otherSections" 
                        :key="sectionKey" 
...
// 计算其他部分（除了basic之外的所有部分）
const otherSections = computed(() => {
  const { basic, ...rest } = specSections
  return rest
})
