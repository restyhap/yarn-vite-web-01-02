 ...
 * @LastEditTime: 2025-01-16 09:55:34
...
const tempEditData = ref<TempEditData>({})
...
const handleEdit = (section: string) => {
  ...
  // 创建临时数据副本
  if (section === 'basic') {
    tempEditData.value = JSON.parse(JSON.stringify(productInfo.value)) as TempEditData
  } else {
    tempEditData.value = JSON.parse(JSON.stringify(specTables.value[section])) as TempEditData
  }
}
...
const handleSave = (section: string) => {
  if (section === 'basic') {
    const basicData = {
      tccode: tempEditData.value.tccode || '',
      supplier: tempEditData.value.supplier || '',
      supplier_code: tempEditData.value.supplier_code || '',
      supplier_name: tempEditData.value.supplier_name || '',
      fire_standard: tempEditData.value.fire_standard || '',
      shipping_port: tempEditData.value.shipping_port || '',
      fob_20_container_price: tempEditData.value.fob_20_container_price || '',
      fob_40_container_price: tempEditData.value.fob_40_container_price || ''
    } as ProductInfo
    productInfo.value = basicData
  } else {
