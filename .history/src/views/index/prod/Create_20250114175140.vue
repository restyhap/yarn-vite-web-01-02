  ...
  <div class="flex-1 p-8">
    <div class="bg-white rounded-lg shadow p-6">
      ...
      <el-steps :active="activeStep" finish-status="success" class="mb-8">
        <el-step title="基本信息" description="产品基本信息" />
        ...
        <el-step title="内外结构" description="内外部结构" />
        <el-step title="扶手信息" description="扶手相关" />
        <el-step title="泡棉信息" description="泡棉相关" />
        <el-step title="脚轮信息" description="脚轮相关" />
        <el-step title="底座信息" description="底座相关" />
        <el-step title="气压棒" description="气压棒相关" />
        <el-step title="气压罩" description="气压罩相关" />
        <el-step title="机构信息" description="机构相关" />
        <el-step title="配件信息" description="配件相关" />
      </el-steps>
...
const rules = ref<FormRules>({
  ...
  upholstery: {
    fabric_manufacturer: [
      // { required: true, message: '请输入面料制造商', trigger: 'blur' }
    ],
    colour_code: [
  ...
  carton: {
    width: [
      // { required: true, message: '请输入包装箱宽度', trigger: 'blur' }
    ],
    depth: [
      // { required: true, message: '请输入包装箱深度', trigger: 'blur' }
    ],
    height: [
      // { required: true, message: '请输入包装箱高度', trigger: 'blur' }
    ]
  },
  logistics: {
    production_time: [
    ...
    loading_quantity_20gp: [
      // { required: true, message: '请输入20尺柜装载量', trigger: 'blur' }
    ],
    loading_quantity_40hc: [
      // { required: true, message: '请输入40尺柜装载量', trigger: 'blur' }
    ]
  },
  dimensions: {
    seat_width: [
      // { required: true, message: '请输入座位宽度', trigger: 'blur' }
    ],
    seat_depth: [
      // { required: true, message: '请输入座位深度', trigger: 'blur' }
    ]
  },
  innerComponents: {
    component_type: [
    ...
    material_code: [
      // { required: true, message: '请输入材料代码', trigger: 'blur' }
    ]
  },
  outerComponents: {
    component_type: [
...
const nextStep = async () => {
  ...
  if (currentFormRef) {
    await currentFormRef.validate((valid: boolean) => {
      if (valid) {
        activeStep.value++
      }
    })
...
// 获取编辑数据
const getEditData = async (productId: string): Promise<void> => {
  try {
    // TODO: 调用获取详情API
    const mockData = {
      basic: {
    ...
    formData.value.basic = { ...mockData.basic }
  } catch (error) {
