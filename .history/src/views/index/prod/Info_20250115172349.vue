 ...
 * @LastEditTime: 2025-01-15 17:23:34
...
import { Document as DocxDocument, Packer, Paragraph, TextRun, Table, TableRow, TableCell, HeadingLevel, AlignmentType, BorderStyle } from 'docx'
    ...
    const doc = new DocxDocument({
      sections: [{
        ...
        children: [
          ...
          // 基本信息表格
          createTable([
            ['TCCODE', productInfo.value.tccode || ''],
            ['SUPPLIER', productInfo.value.supplier || ''],
            ['SUPPLIER CODE', productInfo.value.supplier_code || ''],
            ['SUPPLIER NAME', productInfo.value.supplier_name || ''],
            ['FIRE STANDARD', productInfo.value.fire_standard || ''],
            ['FOB 20\' CONTAINER PRICE', productInfo.value.fob_20_container_price ? `¥${productInfo.value.fob_20_container_price}` : 'N/A'],
            ['FOB 40\'HC CONTAINER PRICE', productInfo.value.fob_40_container_price ? `¥${productInfo.value.fob_40_container_price}` : 'N/A'],
            ['SHIPPING PORT', productInfo.value.shipping_port || '']
          ]),
          ...
          createTable([
            ['FABRIC MANUFACTURER', specTables.value.upholstery?.fabric_manufacturer || 'N/A'],
            ['COLOUR CODE', specTables.value.upholstery?.colour_code || 'N/A'],
            ['LEATHER GRADE (WHERE APPLICABLE)', specTables.value.upholstery?.leather_grade || 'N/A'],
            ['USAGE PER CHAIR (BACK/SEAT)', `${specTables.value.upholstery?.usage_per_chair || 'N/A'}m²`]
          ]),
          ...
          createTable([
            ['WIDTH', `${specTables.value.carton?.width || ''}mm`],
            ['DEPTH', `${specTables.value.carton?.depth || ''}mm`],
            ['HEIGHT', `${specTables.value.carton?.height || ''}mm`],
            ['BOARD TYPE', specTables.value.carton?.board_type || ''],
            ['Items per carton', `${specTables.value.carton?.items_per_carton || ''}pc`],
            ['CARTON VOLUME (m³)', specTables.value.carton?.carton_volume || '']
          ]),
          ...
          createTable([
            ['PRODUCTION TIME (DAYS)', `${specTables.value.logistics?.production_time || ''}days`],
            ['EFFECTIVE VOLUME (m³)', specTables.value.logistics?.effective_volume || ''],
            ['LOADING QUANTITY 20\'GP', specTables.value.logistics?.loading_quantity_20gp || ''],
            ['LOADING QUANTITY 40\'HC', specTables.value.logistics?.loading_quantity_40hc || ''],
            ['NET WEIGHT (KG)', `${specTables.value.logistics?.net_weight || ''}kg`],
            ['GROSS WEIGHT (KG)', `${specTables.value.logistics?.gross_weight || ''}kg`]
          ]),
          ...
          createTable([
            ['SEAT', `WIDTH: ${specTables.value.dimensions?.seat_width || ''}mm DEPTH: ${specTables.value.dimensions?.seat_depth || ''}mm`],
            ['BACK', `WIDTH: ${specTables.value.dimensions?.back_width || ''}mm HEIGHT: ${specTables.value.dimensions?.back_height || ''}mm`],
            ['SEAT HEIGHT', `MIN: ${specTables.value.dimensions?.seat_height_min || ''}mm MAX: ${specTables.value.dimensions?.seat_height_max || ''}mm`],
            ['BACK HEIGHT FROM SEAT', `${specTables.value.dimensions?.back_height_from_seat || ''}mm`],
            ['OVERALL', `WIDTH: ${specTables.value.dimensions?.overall_width || ''}mm DEPTH: ${specTables.value.dimensions?.overall_depth || ''}mm HEIGHT: ${specTables.value.dimensions?.overall_height_min || ''}-${specTables.value.dimensions?.overall_height_max || ''}mm`]
          ])
        ]
      }]
    })
