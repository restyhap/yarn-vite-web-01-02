...
// 格式化章节标题
const formatSectionTitle = (section: keyof SpecTables): string => {
  return section.charAt(0).toUpperCase() + section.slice(1)
}

// 格式化字段标签
const formatFieldLabel = (field: keyof SpecTableSection): string => {
  return field.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')
}

// 编辑功能
const handleEdit = (section: keyof SpecTables) => {
  editingSection.value = section
}

// 保存功能
const handleSave = (section: keyof SpecTables) => {
  editingSection.value = null
  // TODO: 实现保存逻辑
}

// 取消功能
const handleCancel = (section: keyof SpecTables) => {
  editingSection.value = null
}
