 ...
 * @LastEditTime: 2025-01-16 08:22:23
...
type SpecTableValue = string | number

interface SpecTableSection {
  [key: string]: SpecTableValue
}
...
const formatSpecValue = (section: string, field: string): string => {
  ...
  // 根据字段名添加单位
  if (field.includes('density')) return `${stringValue} kg/m³`
  if (field.includes('thickness') || field.includes('height') || field.includes('width') || field.includes('depth')) return `${stringValue} mm`
  if (field.includes('volume')) return `${stringValue} m³`
  if (field.includes('weight')) return `${stringValue} kg`
  if (field.includes('count') || field.includes('quantity')) return `${stringValue} 件`
