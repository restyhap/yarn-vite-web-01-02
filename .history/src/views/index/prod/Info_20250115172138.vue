 ...
 * @LastEditTime: 2025-01-15 17:21:22
    ...
    const doc = new DocxDocument({
      sections: [{
        ...
        children: [
          ...
          createTable([
            ['SEAT', `WIDTH: ${specTables.value.dimensions?.seat_width || ''}mm\nDEPTH: ${specTables.value.dimensions?.seat_depth || ''}mm`],
            ['BACK', `WIDTH: ${specTables.value.dimensions?.back_width || ''}mm\nHEIGHT: ${specTables.value.dimensions?.back_height || ''}mm`],
            ['SEAT HEIGHT', `MIN: ${specTables.value.dimensions?.seat_height_min || ''}mm\nMAX: ${specTables.value.dimensions?.seat_height_max || ''}mm`],
            ['BACK HEIGHT FROM SEAT', `${specTables.value.dimensions?.back_height_from_seat || ''}mm`],
            ['OVERALL', `WIDTH: ${specTables.value.dimensions?.overall_width || ''}mm\nDEPTH: ${specTables.value.dimensions?.overall_depth || ''}mm\nHEIGHT: ${specTables.value.dimensions?.overall_height_min || ''}-${specTables.value.dimensions?.overall_height_max || ''}mm`]
          ])
        ]
      }]
    })
