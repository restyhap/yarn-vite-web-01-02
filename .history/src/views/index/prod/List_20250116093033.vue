          ...
          <template #default="scope">
            <div class="flex items-center justify-center space-x-1">
              <el-button 
                ...
                type="primary" 
                size="small"
                @click="(e) => handleClick(e, scope.row)"
              >
...
const handleClick = (e: MouseEvent, row: any) => {
  ...
  // 如果点击的是图标或文本，向上查找按钮元素
  while (target && !target.classList.contains('view-button') && !target.classList.contains('edit-button')) {
  ...
  if (target.classList.contains('view-button')) {
    router.push(`/index/prod/info/${row.id}`)
  } else if (target.classList.contains('edit-button')) {
    router.push(`/index/prod/edit/${row.id}`)
  }
}
